...
def generate_plot():
    ...
    # The bib file is generated by scripts/fetch_europepmc.py, so a light
    # regex is enough here and avoids depending on a BibTeX parsing library.
    entries = re.findall(r'@\w+\s*\{([^,]*),(.*?)\n\}', bibtex_str, flags=re.DOTALL)
    ...
    for key, body in entries:
        match = re.search(r'\byear\s*=\s*[{"]?\s*(\d{4})', body, flags=re.IGNORECASE)
        if match:
            years.append(int(match.group(1)))
        else:
            print(f"Warning: Entry '{key.strip() or 'Unknown ID'}' has no usable year field. Skipping.")
